...
unsigned char gx_sobelFilter(int w, int h,int check)
{
	int tmp = 0;
	int a, b;
...
void *compute_sobel(void* idx){
    ...
    for (int j = 0; j<imgHeight; j++) {
        for (int i = 0; i<imgWidth; i++){
            pic_gx[j*imgWidth + i] = gx_sobelFilter(i, j,0);
			pic_gy[j*imgWidth + i] = gx_sobelFilter(i, j,1);
...
int main()
{
	...
	for (kk = 0; kk<5; kk++){
		// read input BMP file
		pic_in = bmpReader->ReadBMP(inputfile_name[kk], &imgWidth, &imgHeight);
		...
		bmpReader->WriteBMP(outputMed_name[kk], imgWidth, imgHeight, pic_final);
